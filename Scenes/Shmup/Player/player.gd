...
@onready var cntshot = preload("res://Scenes/Shmup/Projectiles/center_shot.tscn")
...
func _process(_delta):
	...
	stopshooting = Input.is_action_just_released("ButZ")
	shooting = Input.is_action_just_pressed("ButZ")
	eating = Input.is_action_just_pressed("ButX")
	punching = Input.is_action_just_pressed("ButC")
	#print(stopshooting)
	animation()
	evaluate()
		...
		true:
			...
			if ammo > 0:
				...
				match currentgun:
					...
					"Shotty":
						spawner_component.scene = load("res://Scenes/Shmup/Projectiles/trishot.tscn")
					...
					"Laser":
						shootholding = true
						spawner_component.scene = load("res://Scenes/Shmup/Projectiles/Laser.tscn")
